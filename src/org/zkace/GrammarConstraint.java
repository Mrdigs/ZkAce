...
public class GrammarConstraint extends SimpleConstraint {
	...
	public void validate(Component comp, Object value) throws WrongValueException {
		...
		if (comp instanceof Acebox && (strict || lenient)) {
			final Acebox acebox = (Acebox) comp;
			if (acebox.annotations != null && acebox.annotations.size() > 0) {
				final StringBuffer issues = new StringBuffer();
				for (Map<String, Object> annotation : acebox.annotations) {
				...
				if (issues.length() > 0) {
					throw new WrongValueException(acebox, issues.toString());
				}
			}
		}
